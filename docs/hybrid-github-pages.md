...
2. **IAM role for GitHub**: Provide `GITHUB_REPO_OWNER`/`GITHUB_REPO_NAME` (and optionally `GITHUB_SUBJECT_FILTER` or `GITHUB_OIDC_SUBJECT`) plus `GITHUB_OIDC_PROVIDER_ARN` before running `cdk deploy`. Create the provider once per account with the standalone bootstrap CloudFormation stack, then pass its ARN to the CDK app. The stack will provision an IAM role limited to `s3:GetObject` on `aws-actions.json`; the role ARN is exposed as the `GitHubActionsRoleArn` output.
